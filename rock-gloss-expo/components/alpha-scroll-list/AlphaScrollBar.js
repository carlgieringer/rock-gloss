...
export default class AlphaScrollBar extends Component {
  ...
  componentWillReceiveProps (newProps) {
    if (newProps.doReverse !== this.props.doReverse) {
      const alphabet = newProps.doReverse ? [...ALPHABET].reverse() : ALPHABET;
        ...
        {this.state.alphabet.map(letter => (
          <View key={letter}>
            <Text style={{
              ...
              fontSize: RFValue(this.props.isPortrait ? 2 : 1.6) * this.props.fontSizeMultiplier,
            }}>
              {letter}
            </Text>
          </View>
